...
namespace EntityGenerator.Tests
{
    class SqlStructure
    {
        ...
        public SqlStructure()
        {
            _connection = new SqlConnection("Server=192.168.0.200,1499;Database=CloudSky;Uid=sa;Password=Tycad$202;MultipleActiveResultSets=true;Connection Timeout=300;");

            try
            {
                _connection.Open();
                tableLookup = GetTableLookup(_connection, SqlType.MSSql);
                viewLookup = GetViewLookup(_connection, SqlType.MSSql);
                tableComments = GetTableComments(_connection, SqlType.MSSql);
        ...
        public IEnumerable<ColumnInfo> GetViewMySqlColumns(string entity, string viewSql)
        {
        ...
        public IEnumerable<ColumnInfo> GetViewMSSqlColumns(string entity, string viewSql)
        {
            if (_connection.State == ConnectionState.Closed)
            {
                _connection.Open();
